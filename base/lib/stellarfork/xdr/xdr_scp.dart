...
class XdrSCPNomination {
  ...
  static XdrSCPNomination decode(XdrDataInputStream stream) {
    ...
    decodedSCPNomination.votes = List<XdrValue>.generate(votesSize, (i) {
      return XdrValue.decode(stream);
    });
    int acceptedSize = stream.readInt();
    decodedSCPNomination.accepted = List<XdrValue>.generate(acceptedSize, (i) {
      return XdrValue.decode(stream);
    });
...
class XdrSCPQuorumSet {
  ...
  static XdrSCPQuorumSet decode(XdrDataInputStream stream) {
    ...
    decodedSCPQuorumSet._validators = List<XdrPublicKey>.generate(validatorsSize, (i){
      return XdrPublicKey.decode(stream);
    });

    int innerSetsSize = stream.readInt();
    decodedSCPQuorumSet._innerSets = List<XdrSCPQuorumSet>.generate(innerSetsSize, (i){
