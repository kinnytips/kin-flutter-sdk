...
class SubmitTransactionResponse extends Response {
  ...
  int? getOfferIdFromResult(int position) {
    ...
    XdrOperationType? disc = result.result!.results![position].tr!.discriminant;
...
class ExtrasResultCodes {
  ...
  factory ExtrasResultCodes.fromJson(Map<String, dynamic> json) =>
      new ExtrasResultCodes(
        json['transaction'] as String?,
        (json['operations'] as List?)?.map((e) => e as String?).whereType<String>().toList(),
      );
}
